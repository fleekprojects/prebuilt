	...
	$("#settingsform").submit(function(e) { 
		...
		$.ajax({
		  ...
		  success: function (data) {
			if(data == 1){
			  $('#settingsmsg').html('<p><b style="color: green;">Settings Successfully Updated </b></p>');setTimeout(function(){location.reload(); }, 1000);
			}
			else{
	...
	$( "#profileform" ).on( "submit", function(e){
		...
		$.ajax({
		  ...
		  success: function (data) {
			if(data == 1){
			  $('#profilemsg').html('<p><b style="color: green;">Profile Successfully Updated </b></p>');setTimeout(function(){location.reload(); }, 1000);
			}
			else{
	...
	function togglefeature(id,cont){
		$.ajax({
			url : baseurl+'admin/'+cont+"/toggleFeature",
			type: "POST",
			data :{id: id} ,
			success: function(data, textStatus, jqXHR){
			   if(data){
				   $('#msg').html('<p class="alert alert-success" ><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>Record Successfully Updated</p>');
				}
			},
			error: function (xhr, textStatus, errorThrown){
				alert(xhr.responseText)
