	...
	*/
	...
	public function domainsCheck( $domains )
	{
		...
		if ( FALSE === strpos( $domains, ',' ) ) {
			...
			//$is_premium = ( 'true' == strtolower( (string)$this->Response->DomainCheckResult->attributes()->IsPremiumName ) ) ? TRUE : FALSE;
			//$status_of_domain = array('status' => $status,'premium' => $is_premium);
			//return $status_of_domain;
			foreach ( $this->Response->DomainCheckResult as $result ) {
			$domain = (string)$result['Domain'];
			$IsPremiumName = (string)$result['IsPremiumName'];
			$PremiumRegistrationPrice = (string)$result['PremiumRegistrationPrice'];
			$status = ( 'true' == strtolower( (string)$result['Available'] ) ) ? TRUE : FALSE;
			$status_of_domain = array('status' => $status,'domain' => $domain,'IsPremiumName' => $IsPremiumName,'PremiumRegistrationPrice' => $PremiumRegistrationPrice);
			//$r[$domain] = $status;
			}
			return $status_of_domain;
		}
